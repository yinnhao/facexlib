...
def visualize_detection(img, bboxes_and_landmarks, save_path=None, to_bgr=False, return_img=False):
    ...
    for b in bboxes_and_landmarks:
        # confidence
        cv2.putText(img, f'{b[4]:.4f}', (int(b[0]), int(b[1] + 12)), cv2.FONT_HERSHEY_DUPLEX, 2.0, (255, 255, 255))
        # bounding boxes
        b = list(map(int, b))
