...
class InferDET(video_infer):
    def __init__(self, file_name, save_name, encode_params, model=None, scale=1, in_pix_fmt="yuv444p", out_pix_fmt="yuv444p", decode_out_pix_fmt="yuv420p" 
                 ,use_fp16=True, target_size=512, max_size=1024, use_origin_size=False, **decode_param_dict) -> None:
    ...
    def forward(self, x):
        ...
        y = x[:,:,::-1].copy()
        with torch.no_grad():
            vis_img = self.face_enhancer.get_face_parsing(y)
...
if __name__ == '__main__':
    ...
    infer = InferDET(video_path, save_path, encode_params, in_pix_fmt="rgb24", out_pix_fmt="rgb24", 
                           decode_out_pix_fmt="yuv420p", scale=1, target_size=target_size, max_size=max_size, use_origin_size=use_origin_size)
    infer.infer()
